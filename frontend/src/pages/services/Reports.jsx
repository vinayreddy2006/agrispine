...
const Reports = () => {
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-agriBg">
    ...
    const cropPerformance = crops.map(crop => {
        ...
        // Translation logic for crops (Matches JSON 'crops_list')
        const translatedName = t(`crops_list.${crop.cropName.toLowerCase()}`, { defaultValue: crop.cropName });
            ...
            <div className="max-w-6xl mx-auto px-6 mt-8 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">{t('reports.total_invest')}</p>
                        <h3 className="text-2xl font-bold text-gray-800 mt-1">₹{totalInvestment.toLocaleString('en-IN')}</h3>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">{t('reports.total_rev')}</p>
                    ...
                    <div className={`p-6 rounded-2xl shadow-sm border ${netProfit >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                        <p className={`text-xs font-bold uppercase tracking-wider ${netProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>{t('reports.net_profit')}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col h-96">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-bold text-gray-700">{t('reports.profit_by_crop')}</h3>
                        </div>
                                ...
                                {cropPerformance.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={cropPerformance} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                            ...
                                            <YAxis hide />
                                            <Tooltip formatter={(value) => `₹${value.toLocaleString('en-IN')}`} cursor={{ fill: 'transparent' }} contentStyle={{ borderRadius: '10px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
                                    ...
                                    <div className="h-full flex items-center justify-center text-gray-400 text-sm">{t('reports.no_data')}</div>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col h-96">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-bold text-gray-700">{t('reports.expense_breakdown')}</h3>
                        </div>
                        <div className="flex-1 w-full relative min-h-0">
                            <div className="absolute inset-0">
                                {expenseData.length > 0 ? (
                                    ...
                                    <div className="h-full flex items-center justify-center text-gray-400 text-sm">{t('reports.no_exp')}</div>
                                )}
                            </div>
                        </div>
                    </div>
