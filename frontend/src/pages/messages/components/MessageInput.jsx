        ...
        <div className="bg-[#f0f2f5] px-2 md:px-4 py-2 z-20 shrink-0 pb-safe relative shadow-[0_-1px_3px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex items-center gap-2">
                {isRecording ? (
                    ...
                    <>
                        <button type="button" onClick={() => { setShowInputEmoji(!showInputEmoji); setShowAttachmentMenu(false); }} className={`p-2.5 rounded-full transition-all flex items-center justify-center ${showInputEmoji ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 scale-105' : 'text-slate-700 hover:text-green-700 hover:bg-slate-200 bg-slate-200/50 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-green-400'}`}><Smile className="w-5 h-5" /></button>
                        
                        <div className="relative">
                            <button type="button" onClick={() => { setShowAttachmentMenu(!showAttachmentMenu); setShowInputEmoji(false); }} className={`p-2.5 rounded-full transition-all flex items-center justify-center ${imagePreview ? 'text-green-700 bg-green-100 dark:bg-green-900/30 dark:text-green-400 scale-105' : showAttachmentMenu ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 scale-105' : 'text-slate-700 hover:text-green-700 hover:bg-slate-200 bg-slate-200/50 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-green-400'}`}><Paperclip className="w-5 h-5" /></button>
                            {showAttachmentMenu && (
